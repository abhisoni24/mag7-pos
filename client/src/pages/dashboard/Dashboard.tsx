...
const getActivityIcon = (type: string) => {
  switch (type) {
    case 'new_order':
      return <div className="rounded-full bg-primary-light bg-opacity-20 p-2 mr-3">
        <UtensilsCrossed className="h-5 w-5 text-primary" />
      </div>;
    case 'completed_order':
      return <div className="rounded-full bg-success bg-opacity-20 p-2 mr-3">
    ...
    case 'payment':
      return <div className="rounded-full bg-warning bg-opacity-20 p-2 mr-3">
        <CreditCard className="h-5 w-5 text-warning" />
      </div>;
    case 'new_customers':
      return <div className="rounded-full bg-host bg-opacity-20 p-2 mr-3">
...
const Dashboard = () => {
  ...
  const activeTablesPercent = totalTables > 0 ? (activeTables / totalTables) * 100 : 0;
  ...
  const todaySales = orders
    .filter(order => order.status === 'paid' && new Date(order.updatedAt).toDateString() === new Date().toDateString())
    .reduce((total, order) => {
    ...
    orders
      ...
      .forEach(order => {
        recentActivities.push({
          ...
          detail: `Table ${tables.find(t => t._id === order.tableId)?.number || '-'} - Order #${order._id?.substring(0, 4)}`,
        ...
        <Card>
          <CardContent className="pt-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-primary text-lg font-semibold">{activeTables}</span>
            </div>
            <Progress value={activeTablesPercent} className="h-2.5 mb-2" />
            <p className="text-sm text-gray-500">{activeTables} of {totalTables} tables occupied</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="pt-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-warning text-lg font-semibold">{openOrders}</span>
            </div>
        ...
        <Card>
          <CardContent className="pt-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-success text-lg font-semibold">${todaySales.toFixed(2)}</span>
            </div>
            <div className="text-sm text-gray-500">
              ...
              <Progress value={salesPercent} className="h-2.5 bg-gray-200" indicatorClassName="bg-success" />
            </div>
          </CardContent>
        </Card>
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {currentOrders.map(order => {
                const tableNumber = tables.find(t => t._id === order.tableId)?.number || '-';
